...
class App(QWidget):
    ...
    def system_card(self,title,value,detail,color,key):
        card=QFrame(); card.setObjectName("systemCard"); card.setStyleSheet(f"QFrame#systemCard{{background:#ffffff;border:1px solid #e2e8f0;border-top:3px solid {color};border-radius:11px}} QLabel{{background:transparent}}"); layout=QVBoxLayout(card); layout.setContentsMargins(13,10,13,9); layout.setSpacing(3)
        head=QHBoxLayout(); label=QLabel(title); label.setStyleSheet("color:#334155;font-size:11px;font-weight:700"); head.addWidget(label); head.addStretch(); indicator=QLabel("实时"); indicator.setStyleSheet(f"color:{color};background:{color}18;padding:2px 6px;border-radius:4px;font-size:9px;font-weight:700"); head.addWidget(indicator); layout.addLayout(head)
        number=QLabel(value); number.setFont(QFont("Noto Sans CJK SC",22,QFont.Bold)); number.setStyleSheet(f"color:{color}"); layout.addWidget(number)
    ...
    def system_panel(self):
        ...
        panel=QFrame(); panel.setObjectName("systemPanel"); panel.setStyleSheet("QFrame#systemPanel{background:#f1f5f9;border:1px solid #e2e8f0;border-radius:13px} QLabel{background:transparent}"); outer=QVBoxLayout(panel); outer.setContentsMargins(14,14,14,15); outer.setSpacing(11)
        hero=QFrame(); hero.setObjectName("systemHero"); hero.setStyleSheet("QFrame#systemHero{background:#111827;border-radius:11px} QLabel{background:transparent}"); hero_layout=QHBoxLayout(hero); hero_layout.setContentsMargins(14,11,14,11); title_box=QVBoxLayout(); title=QLabel("系统监控"); title.setFont(QFont("Noto Sans CJK SC",17,QFont.Bold)); title.setStyleSheet("color:#f8fafc"); title_box.addWidget(title); subtitle=QLabel("本机资源 · 2 秒采样 · 只保留内存中的最近曲线"); subtitle.setStyleSheet("color:#94a3b8;font-size:10px"); title_box.addWidget(subtitle); hero_layout.addLayout(title_box); hero_layout.addStretch(); live=QLabel("●  实时采集"); live.setStyleSheet("color:#a7f3d0;background:#064e3b;padding:6px 9px;border-radius:6px;font-size:10px;font-weight:700"); hero_layout.addWidget(live); outer.addWidget(hero)
        ...
        details=QFrame(); details.setStyleSheet("background:white;border:1px solid #e2e8f0;border-radius:9px"); detail_grid=QGridLayout(details); detail_grid.setContentsMargins(12,10,12,10); detail_grid.setHorizontalSpacing(28)
        ...
        if gpus:
            gpu_box=QFrame(); gpu_box.setStyleSheet("background:#fff7ed;border:1px solid #fed7aa;border-radius:9px"); gpu_layout=QVBoxLayout(gpu_box); gpu_layout.setContentsMargins(12,9,12,9); gpu_title=QLabel("GPU 详情"); gpu_title.setStyleSheet("color:#9a3412;font-weight:700"); gpu_layout.addWidget(gpu_title)
            for gpu in gpus:
        ...
        process_box=QFrame(); process_box.setStyleSheet("background:white;border:1px solid #e2e8f0;border-radius:9px"); process_layout=QVBoxLayout(process_box); process_layout.setContentsMargins(12,9,12,9); process_title=QLabel("高占用进程"); process_title.setStyleSheet("color:#334155;font-weight:700"); process_layout.addWidget(process_title); table=QTableWidget(min(5,len(metrics.get('processes',[]))),4); table.setHorizontalHeaderLabels(["进程","CPU","内存","PID"]); table.verticalHeader().setVisible(False); table.setEditTriggers(QAbstractItemView.NoEditTriggers); table.setSelectionMode(QAbstractItemView.NoSelection); table.setFocusPolicy(Qt.NoFocus); table.setShowGrid(False); table.setMinimumHeight(150); table.setStyleSheet("QTableWidget{background:white;border:0;color:#334155;font-size:10px} QHeaderView::section{background:#f8fafc;color:#64748b;border:0;padding:5px;font-size:10px;font-weight:700}"); table.horizontalHeader().setSectionResizeMode(0,QHeaderView.Stretch)
    ...
    def account_panel(self):
        all_profiles=profiles(); available=sum(1 for p in all_profiles if ((p.get("limits",{}).get("primary") or {}).get("remainingPercent") is None or (p.get("limits",{}).get("primary") or {}).get("remainingPercent",0)>0)); no_quota=len(all_profiles)-available
        p=QFrame(); p.setObjectName("quotaPanel"); p.setStyleSheet("QFrame#quotaPanel{background:#f8fafc;border:1px solid #e2e8f0;border-radius:13px} QLabel{background:transparent}"); v=QVBoxLayout(p); v.setContentsMargins(14,13,14,14); v.setSpacing(10)
        head=QHBoxLayout(); heading=QLabel("账号额度"); heading.setFont(QFont("Noto Sans CJK SC",15,QFont.Bold)); heading.setStyleSheet("color:#0f172a"); head.addWidget(heading); sub=QLabel("每个账号的重置窗口"); sub.setStyleSheet("color:#64748b;font-size:11px"); head.addWidget(sub); head.addStretch(); summary=QLabel(f"可用 {available}  ·  需刷新 {no_quota}"); summary.setStyleSheet(f"color:{'#b45309' if no_quota else '#047857'};background:{'#fef3c7' if no_quota else '#dcfce7'};padding:5px 9px;border-radius:6px;font-size:11px;font-weight:700"); head.addWidget(summary); v.addLayout(head)
        grid=QGridLayout(); grid.setSpacing(9)
        for index,profile in enumerate(all_profiles):
            limits=profile.get("limits",{}); primary=limits.get("primary") or {}; secondary=limits.get("secondary") or {}; card=QFrame(); card.setObjectName("quotaCard"); card.setStyleSheet("QFrame#quotaCard{background:white;border:1px solid #e2e8f0;border-radius:10px} QLabel{background:transparent}"); c=QVBoxLayout(card); c.setContentsMargins(11,10,11,10); c.setSpacing(7)
            title_row=QHBoxLayout(); name=QLabel(profile.get("name","未命名")); name.setFont(QFont("Noto Sans CJK SC",12,QFont.Bold)); name.setStyleSheet("color:#0f172a"); title_row.addWidget(name); title_row.addStretch(); plan=QLabel((profile.get("planType") or "plus").upper()); plan.setStyleSheet("color:#6366f1;background:#eef2ff;padding:3px 6px;border-radius:4px;font-size:9px;font-weight:700"); title_row.addWidget(plan); c.addLayout(title_row)
            for label,window,accent in (("5 小时",primary,"#2563eb"),("每周",secondary,"#7c3aed")):
                row=QFrame(); row.setStyleSheet("background:#f8fafc;border-radius:7px"); rv=QVBoxLayout(row); rv.setContentsMargins(8,6,8,6); rv.setSpacing(3); line=QHBoxLayout(); caption=QLabel(label); caption.setStyleSheet("color:#475569;font-size:10px;font-weight:700"); line.addWidget(caption); line.addStretch(); value=window.get("remainingPercent"); number=QLabel(f"{value:.0f}% 可用" if isinstance(value,(int,float)) else "--"); number.setStyleSheet(f"color:{accent};font-size:11px;font-weight:700"); line.addWidget(number); rv.addLayout(line); used=window.get("usedPercent"); reset=QLabel(f"已用 {used:.0f}%  ·  {remaining_label(window.get('resetsAt'))}  ·  {reset_label(window.get('resetsAt'))}" if isinstance(used,(int,float)) else "等待读取重置信息"); reset.setStyleSheet("color:#64748b;font-size:9px"); rv.addWidget(reset); bar=QProgressBar(); bar.setTextVisible(False); bar.setRange(0,100); bar.setValue(max(0,min(100,int(value or 0)))); bar.setStyleSheet(f"QProgressBar{{height:5px;border:0;border-radius:2px;background:#e2e8f0}} QProgressBar::chunk{{background:{accent};border-radius:2px}}"); rv.addWidget(bar); c.addWidget(row)
            window_count=sum(bool(window) for window in (primary,secondary)); footer=QLabel(f"重置窗口 {window_count} 个"); footer.setStyleSheet("color:#94a3b8;font-size:9px"); c.addWidget(footer)
            grid.addWidget(card,index//2,index%2)
        v.addLayout(grid); self.box.addWidget(p)
    def learning_panel(self):
        panel=QFrame(); panel.setObjectName("learningPanel"); panel.setStyleSheet("QFrame#learningPanel{background:#f5f7ff;border:1px solid #e0e7ff;border-radius:13px} QLabel{background:transparent}"); v=QVBoxLayout(panel); v.setContentsMargins(14,14,14,15); v.setSpacing(10)
        needs_review=[w for w in self.windows if w.get("completed",0)>self.seen.get(w.get("path",""),0)]; state_text=f"● {self.running_count} 运行中 · {len(needs_review)} 待处理"; hero=QFrame(); hero.setObjectName("learningHero"); hero.setMinimumHeight(64); hero.setStyleSheet("QFrame#learningHero{background:#312e81;border-radius:11px} QLabel{background:transparent}"); hero_layout=QHBoxLayout(hero); hero_layout.setContentsMargins(14,11,14,11); title_box=QVBoxLayout(); title=QLabel("等待学习"); title.setFont(QFont("Noto Sans CJK SC",17,QFont.Bold)); title.setStyleSheet("color:#ffffff"); title_box.addWidget(title); hero_sub=QLabel("把 Codex 的等待时间，变成有边界的机器人前沿学习"); hero_sub.setStyleSheet("color:#c7d2fe;font-size:10px"); title_box.addWidget(hero_sub); hero_layout.addLayout(title_box); hero_layout.addStretch(); state=QLabel(state_text); state.setStyleSheet(f"color:{'#fecaca' if needs_review else '#c7d2fe'};background:{'#7f1d1d' if needs_review else '#4338ca'};padding:5px 9px;border-radius:6px;font-weight:700;font-size:10px"); hero_layout.addWidget(state); v.addWidget(hero)
        if needs_review:
            ...
            v.addWidget(alert)
        control_box=QFrame(); control_box.setMinimumHeight(42); control_box.setStyleSheet("background:white;border:1px solid #e0e7ff;border-radius:9px"); controls=QHBoxLayout(control_box); controls.setContentsMargins(9,7,9,7); hint=QLabel("当前推荐："+self.learning_context.get("label","机器人前沿")); hint.setStyleSheet("color:#4338ca;font-size:11px;font-weight:700"); controls.addWidget(hint); controls.addStretch(); note=QLabel("只读增量"); note.setStyleSheet("color:#94a3b8;font-size:10px"); controls.addWidget(note); controls.addSpacing(5); controls.addWidget(QLabel("学习时长"))
        ...
        refresh=QPushButton("获取最新"); refresh.setEnabled(not (self.feed_future and not self.feed_future.done())); refresh.setStyleSheet("background:#4f46e5;color:white;border:0;font-weight:700;padding:7px 12px"); refresh.clicked.connect(self.start_learning_feed); controls.addWidget(refresh); v.addWidget(control_box)
        ...
        for item in self.feed_items:
            accent="#ef4444" if item.get("kind")=="video" else ("#f59e0b" if item.get("kind")=="model" else "#6366f1"); card=QFrame(); card.setObjectName("learningCard"); card.setMinimumHeight(126); card.setStyleSheet(f"QFrame#learningCard{{background:white;border:1px solid #e0e7ff;border-left:4px solid {accent};border-radius:9px}} QLabel{{background:transparent}}"); c=QVBoxLayout(card); c.setContentsMargins(11,10,12,10); c.setSpacing(6)
            top=QHBoxLayout(); name=QLabel(item.get("title","未命名内容")); name.setWordWrap(True); name.setFont(QFont("Noto Sans CJK SC",13,QFont.Bold)); name.setStyleSheet("color:#111827"); top.addWidget(name,1); badge_text="▶ Demo 视频" if item.get("kind")=="video" else ("◆ 开源模型" if item.get("kind")=="model" else f"{item.get('seconds',45)} 秒读完"); seconds=QLabel(badge_text); seconds.setStyleSheet(f"color:{accent};background:{accent}18;padding:3px 7px;border-radius:5px;font-size:10px;font-weight:700"); top.addWidget(seconds); c.addLayout(top)
            ...
            reason=QLabel("推荐理由："+item.get("recommend_reason","机器人前沿探索")); reason.setWordWrap(True); reason.setStyleSheet("color:#047857;background:#f0fdf4;padding:5px 8px;border-radius:5px;font-size:10px;font-weight:600"); c.addWidget(reason)
            summary=QLabel("解决什么："+item.get("summary","")); summary.setWordWrap(True); summary.setStyleSheet("color:#1e293b;font-size:12px;font-weight:700"); c.addWidget(summary)
            delta=QLabel("相对已有工作："+item.get("delta","")); delta.setWordWrap(True); delta.setStyleSheet("color:#4f46e5;background:#eef2ff;padding:4px 7px;border-radius:5px;font-size:10px"); c.addWidget(delta)
            why=QLabel("为什么值得看："+item.get("why","")); why.setWordWrap(True); why.setStyleSheet("color:#64748b;font-size:10px"); c.addWidget(why)
        ...
        stats=self.feed_stats; storage=QLabel(f"本地存储  {stats.get('count',0)}/{stats.get('limit',1000)}  ·  收藏 {stats.get('saved',0)}  ·  {stats.get('bytes',0)/1024/1024:.1f} MB  ·  普通记录60天后清理"); storage.setAlignment(Qt.AlignCenter); storage.setStyleSheet("color:#64748b;background:#eef2ff;border-radius:6px;font-size:10px;padding:6px"); v.addWidget(storage)
