...
COLORS=dict(zip(STATES,["#6f9fe8","#d99a5b","#4f9b7c","#d66f7b","#64748b"]))
...
class Sparkline(QWidget):
    def __init__(self,color="#6f9fd2",parent=None):
...
def scan():
    ...
    for line in s.splitlines():
        ...
        bridge_info=bridge_window_info(path,name) or initial_bridge; provider=bridge_info.get("provider","subscription")
...
class LessonChatDialog(QDialog):
    ...
    def __init__(self,parent,bundle,concept,lesson,store,executor):
        ...
        self.setWindowTitle("AI 学习助手 · "+concept.get("title_zh","当前知识点")); self.setWindowFlag(Qt.WindowStaysOnTopHint,True); self.setMinimumSize(460,480); self.resize(540,680); self.setStyleSheet(dialog_stylesheet("learn"))
        ...
        composer=QHBoxLayout(); composer.setSpacing(8); self.input=QPlainTextEdit(); self.input.setPlaceholderText("哪里不懂就直接问，例如：这里的速度场是谁预测的？\n支持系统中文输入法；点击发送或按 Ctrl+Enter。") ; self.input.setFixedHeight(82); composer.addWidget(self.input,1); self.send=QPushButton("发送"); self.send.setFixedSize(76,82); self.send.setStyleSheet(button_style("learn",True)); self.send.clicked.connect(self.send_question); composer.addWidget(self.send); root.addLayout(composer)
...
class ProjectIdeasDialog(QDialog):
    def __init__(self, store, project, parent=None):
        ...
        self.setStyleSheet(dialog_stylesheet("learn"))
...
class ConversationDetailsDialog(QDialog):
    def __init__(self,project,conversations,parent=None):
        ...
        self.setStyleSheet(dialog_stylesheet("monitor"))
...
class NetworkDetailsDialog(QDialog):
    def __init__(self,network,parent=None):
        ...
        self.setStyleSheet(dialog_stylesheet("system")+" QFrame#networkCard{background:#fffefa;border:1px solid #e7ded5;border-radius:12px} QPlainTextEdit{font-family:'Noto Sans Mono CJK SC';font-size:10px}"); outer=QVBoxLayout(self); outer.setContentsMargins(16,14,16,16); outer.setSpacing(10); title=QLabel("网络详情与诊断"); title.setFont(QFont("Noto Sans CJK SC",17,QFont.Bold)); title.setStyleSheet("color:#31483f"); outer.addWidget(title)
        proxy=network.get("proxy",{}); card=QFrame(); card.setObjectName("networkCard"); grid=QGridLayout(card); grid.setContentsMargins(13,11,13,12); rows=(("实时下载",format_rate(network.get("download",0))),("实时上传",format_rate(network.get("upload",0))),("当前网卡",network.get("interface","未知网卡")),("Mihomo / Clash","运行中" if proxy.get("running") else "未运行"),("当前节点",proxy.get("node") or "控制接口不可读"),("TUN","已开启" if proxy.get("tun") is True else ("未开启" if proxy.get("tun") is False else "状态未知")),("系统代理",str(proxy.get("system_proxy") or "未知")),("终端代理","已设置" if proxy.get("terminal_proxy") else "未设置"))
        grid.setColumnStretch(1,1)
        for index,(label,value) in enumerate(rows):
            caption=QLabel(label); caption.setStyleSheet("color:#64748b;font-size:10px"); number=QLabel(value); number.setWordWrap(True); number.setMinimumWidth(0); number.setSizePolicy(QSizePolicy.Ignored,QSizePolicy.Preferred); number.setTextInteractionFlags(Qt.TextSelectableByMouse); number.setStyleSheet("color:#0f172a;font-weight:700"); grid.addWidget(caption,index,0); grid.addWidget(number,index,1)
        outer.addWidget(card); tools=QHBoxLayout(); self.diagnose=QPushButton("网络诊断"); self.speed=QPushButton("当前线路测速"); self.diagnose.setStyleSheet("QPushButton{background:#edf9f4;color:#43856e;border:1px solid #c9eadc;border-radius:9px;padding:8px;font-weight:700} QPushButton:hover{background:#e2f5ed}"); self.speed.setStyleSheet("QPushButton{background:#f5f2ff;color:#7668b3;border:1px solid #ddd5f3;border-radius:9px;padding:8px;font-weight:700} QPushButton:hover{background:#eee9ff}"); self.diagnose.clicked.connect(lambda:self.start_job("diagnose",diagnose_network)); self.speed.clicked.connect(lambda:self.start_job("speed",benchmark_current_route)); tools.addWidget(self.diagnose); tools.addWidget(self.speed); outer.addLayout(tools)
        accelerate=QHBoxLayout(); self.codex_boost=QPushButton("Codex 加速"); self.code_boost=QPushButton("代码下载加速"); self.model_boost=QPushButton("模型下载加速"); boost_styles=("QPushButton{background:#eef5ff;color:#4778b8;border:1px solid #c9dcf4;border-radius:9px;padding:8px 10px;font-weight:700} QPushButton:hover{background:#e2efff}","QPushButton{background:#eff9ff;color:#3d829f;border:1px solid #c7e8f2;border-radius:9px;padding:8px 10px;font-weight:700} QPushButton:hover{background:#e2f6fb}","QPushButton{background:#f5f2ff;color:#7668b3;border:1px solid #ddd5f3;border-radius:9px;padding:8px 10px;font-weight:700} QPushButton:hover{background:#eee9ff}");
        for button,style in zip((self.codex_boost,self.code_boost,self.model_boost),boost_styles): button.setStyleSheet(style); button.setMinimumHeight(38)
        self.codex_boost.clicked.connect(lambda:self.start_job("accelerate:Codex",lambda:switch_fastest("Codex"))); self.code_boost.clicked.connect(lambda:self.start_job("accelerate:GitHub",lambda:switch_fastest("GitHub"))); self.model_boost.clicked.connect(lambda:self.start_job("accelerate:Hugging Face",lambda:switch_fastest("Hugging Face"))); accelerate.addWidget(self.codex_boost); accelerate.addWidget(self.code_boost); accelerate.addWidget(self.model_boost); outer.addLayout(accelerate)
        self.output=QPlainTextEdit(); self.output.setReadOnly(True); self.output.setLineWrapMode(QPlainTextEdit.WidgetWidth); self.output.setHorizontalScrollBarPolicy(Qt.ScrollBarAlwaysOff); self.output.setPlainText("三个“加速”按钮会实测对应服务，并直接切换相关 Mihomo 策略组。\n当前线路测速约下载 3 MB、上传 128 KB。\n所有操作仅在点击后执行。") ; outer.addWidget(self.output,1); close=QPushButton("关闭"); close.clicked.connect(self.accept); outer.addWidget(close,0,Qt.AlignRight)
...
class App(QWidget):
    def __init__(self):
        ...
        self.shell=QFrame(); self.shell.setObjectName("shell"); self.shell.setStyleSheet("QFrame#shell{background:#fbfaf7;border:1px solid #e7ded5;border-radius:14px}"); shell_layout=QVBoxLayout(self.shell); shell_layout.setContentsMargins(0,0,0,0); shell_layout.setSpacing(0)
        self.header=DraggableHeader(); self.header.setStyleSheet("background:#fffdf9;border:0;border-bottom:1px solid #eee5dc;border-top-left-radius:14px;border-top-right-radius:14px"); h=QHBoxLayout(self.header); h.setContentsMargins(10,6,8,6); h.setSpacing(5); mascot=QLabel(); mascot.setFixedSize(34,34); mascot_pixmap=QPixmap(str(ASSETS_DIR/"mascots/control-cat.png")); mascot.setPixmap(mascot_pixmap.scaled(34,34,Qt.KeepAspectRatio,Qt.SmoothTransformation)); mascot.setToolTip("控制台猫猫正在值班"); h.addWidget(mascot); title=QLabel("Codex 控制塔"); title.setFont(QFont("Noto Sans CJK SC",14,QFont.Bold)); title.setStyleSheet("color:#253047;border:0"); h.addWidget(title)
        self.monitor_tab=QPushButton("任务"); self.todo_tab=QPushButton("待办"); self.learn_tab=QPushButton("学习"); self.system_tab=QPushButton("系统")
    ...
    def system_panel(self):
        ...
        panel=QFrame(); panel.setObjectName("systemPanel"); panel.setStyleSheet(panel_style("system")); outer=QVBoxLayout(panel); outer.setSizeConstraint(QLayout.SetMinimumSize); outer.setContentsMargins(14,12,14,14); outer.setSpacing(8)
        ...
        overview=QFrame(); overview.setObjectName("systemOverview"); overview.setStyleSheet("QFrame#systemOverview{background:#fffefa;border:1px solid #dce9e2;border-radius:12px}"); metrics_row=QHBoxLayout(overview); metrics_row.setContentsMargins(0,3,0,3); metrics_row.setSpacing(0)
        cards=(("CPU",f"{cpu.get('percent',0):.0f}%",f"{cpu.get('cores',0)} 核 · 负载 {cpu.get('load',0):.2f}{temp}","#5c8fc5","cpu"),("内存",f"{memory.get('percent',0):.0f}%",memory_detail,"#8a76c0","memory"),("GPU / 显存",gpu_value,gpu_detail,"#cf8262","gpu"),("磁盘",disk_value,disk_detail,"#4f9b7c","disk"))
        ...
        details=QFrame(); details.setObjectName("systemDetails"); details.setMinimumHeight(62); details.setStyleSheet("QFrame#systemDetails{background:white;border:1px solid #e2e8f0;border-radius:9px}"); detail_grid=QGridLayout(details); detail_grid.setContentsMargins(12,7,12,8); detail_grid.setHorizontalSpacing(24); detail_grid.setColumnStretch(0,3); detail_grid.setColumnStretch(1,2); detail_grid.setColumnStretch(2,2)
        proxy=network.get("proxy",{}); proxy_name=(proxy.get("node") or ("已运行" if proxy.get("running") else "未运行")); network_value=QWidget(); network_value.setMinimumWidth(0); network_line=QHBoxLayout(network_value); network_line.setContentsMargins(0,0,0,0); network_line.setSpacing(8); speed=QLabel(f"↓ {format_rate(network.get('download',0))}   ↑ {format_rate(network.get('upload',0))} · {network.get('interface','未知网卡')} · {proxy_name}"); speed.setMinimumWidth(0); speed.setSizePolicy(QSizePolicy.Ignored,QSizePolicy.Preferred); speed.setStyleSheet("color:#334155;font-size:10px;font-weight:600"); speed.setToolTip(speed.text()); network_line.addWidget(speed,1); network_more=QPushButton("网络详情"); network_more.setCursor(Qt.PointingHandCursor); network_more.setFixedSize(78,30); network_more.setStyleSheet("QPushButton{background:#edf9f4;color:#43856e;border:1px solid #c9eadc;border-radius:8px;padding:0;font-size:10px;font-weight:700} QPushButton:hover{background:#e2f5ed;border-color:#a8dbc7} QPushButton:pressed{background:#d7eee4}"); network_more.clicked.connect(self.open_network_details); network_line.addWidget(network_more)
        ...
        for i in range(3):detail_grid.itemAtPosition(0,i).widget().setStyleSheet("color:#64748b;font-size:10px;font-weight:700")
        for i in (1,2):detail_grid.itemAtPosition(1,i).widget().setStyleSheet("color:#0f172a;font-size:11px;font-weight:600")
    ...
    def window_panel(self):
        p=QFrame(); p.setObjectName("monitorPanel"); p.setStyleSheet(panel_style("monitor")); v=QVBoxLayout(p); v.setContentsMargins(14,12,14,14); v.setSpacing(8)
        head=QHBoxLayout(); heading=QLabel("项目窗口"); heading.setFont(QFont("Noto Sans CJK SC",15,QFont.Bold)); heading.setStyleSheet("color:#0f172a"); head.addWidget(heading); head.addStretch(); running=QLabel(f"●  正在运行 {self.running_count}"); running.setStyleSheet("color:#4778b8;background:#eaf3ff;padding:4px 9px;border-radius:6px;font-size:11px;font-weight:700"); head.addWidget(running); done=QLabel(f"✓  已完成 {self.done_count}"); done.setStyleSheet("color:#3f8a70;background:#e9f7f0;padding:4px 9px;border-radius:6px;font-size:11px;font-weight:700"); head.addWidget(done); v.addLayout(head)
        ...
        for w in self.windows:
            ...
            card=QFrame(); card.setObjectName("windowCard"); card.setStyleSheet("QFrame#windowCard{background:#fffefa;border:1px solid #d7e6f3;border-radius:12px}"); card_layout=QVBoxLayout(card); card_layout.setContentsMargins(12,10,10,10); card_layout.setSpacing(8); h=QHBoxLayout(); h.setSpacing(6)
            dot=QLabel("●" if unread else ("●" if state=="正在运行" else "○")); dot.setStyleSheet(f"color:{'#e56f7c' if unread else ('#78a7e8' if state=='正在运行' else '#94a3b8')};font-size:18px"); h.addWidget(dot)
            ...
            chats=QToolButton(); chats.setText(f"详情 {total_conversations}  ▾"); chats.setPopupMode(QToolButton.InstantPopup); chats.setCursor(Qt.PointingHandCursor); chats.setToolTip("查看当前对话数据和这个项目的其他对话"); chats.setStyleSheet(button_style("monitor"))
            ...
            ideas=QPushButton(f"灵感 {len(open_ideas)}" if open_ideas else "记灵感"); ideas.setToolTip("记录和管理这个项目暂未实现的想法"); ideas.setStyleSheet(button_style("learn")); ideas.clicked.connect(lambda _,x=w:self.open_project_ideas(x)); h.addWidget(ideas)
            ...
            focus=QPushButton("切换并聚焦" if pending else ("查看" if unread else "聚焦")); focus.setStyleSheet((button_style("todo",True) if pending else ("QPushButton{background:#d66f7b;color:white;border:0;border-radius:9px;font-weight:700} QPushButton:hover{background:#bf5e6a}" if unread else button_style("monitor",True)))); focus.clicked.connect(lambda _,x=w:self.focus(x["id"])); h.addWidget(focus); card_layout.addLayout(h)
            if metric_values:
                strip=QFrame(); strip.setObjectName("tokenStrip"); strip.setStyleSheet("QFrame#tokenStrip{background:#f4f8fe;border:1px solid #e2ecf8;border-radius:7px} QLabel{background:transparent;border:0}"); metric_row=QHBoxLayout(strip); metric_row.setContentsMargins(10,6,10,6); metric_row.setSpacing(0)
                for index,(label,value) in enumerate(metric_values):
                    cell=QWidget(); cell_layout=QHBoxLayout(cell); cell_layout.setContentsMargins(7,0,7,0); cell_layout.setSpacing(5); caption=QLabel(label); caption.setStyleSheet("color:#64748b;font-size:9px;font-weight:600"); number=QLabel(value); number.setStyleSheet("color:#4778b8;font-size:11px;font-weight:700"); cell_layout.addWidget(caption); cell_layout.addWidget(number); cell_layout.addStretch(); metric_row.addWidget(cell,1)
    ...
    def learning_panel(self):
        panel=QFrame(); panel.setObjectName("learnPanel"); panel.setStyleSheet(panel_style("learn")); v=QVBoxLayout(panel); v.setContentsMargins(14,12,14,14); v.setSpacing(8)
        ...
        if needs_review:
            ...
            v.addWidget(alert)
        mode_bar=QFrame(); mode_bar.setObjectName("learningModeBar"); mode_bar.setStyleSheet("QFrame#learningModeBar{background:#eeeaf4;border:0;border-radius:11px}"); mode_row=QHBoxLayout(mode_bar); mode_row.setContentsMargins(4,4,4,4); mode_row.setSpacing(4)
        for mode,label in (("frontier","前沿追踪"),("curriculum","系统学习"),("vocabulary","单词闪卡")):
            button=QPushButton(label); active=self.learning_mode==mode; button.setStyleSheet("background:#fffefa;color:#685a94;border:1px solid #d8cef0;font-weight:700" if active else "background:transparent;color:#716d79;border:0"); button.clicked.connect(lambda _,value=mode:self.switch_learning_mode(value)); mode_row.addWidget(button)
        ...
        refresh=QPushButton("获取最新"); refresh.setEnabled(not (self.feed_future and not self.feed_future.done())); refresh.setStyleSheet("QPushButton{background:#8f7bc2;color:white;border:0;border-radius:9px;font-weight:700} QPushButton:hover{background:#806caf} QPushButton:disabled{background:#d8d1e8}"); refresh.clicked.connect(self.start_learning_feed); controls.addWidget(refresh); v.addWidget(control_bar)
        ...
        channel_colors={"core":("#5f8fd6","#f0f6ff"),"adjacent":("#8b78c9","#f7f4ff"),"major":("#d66f7b","#fff3f4"),"emerging":("#c58a3a","#fff9ec"),"serendipity":("#4f9b7c","#effaf5")}; kind_labels={"paper":"论文","repo":"代码","model":"模型","video":"Demo"}
    ...
    def vocabulary_panel(self,v,panel):
        ...
        if not self.vocab_revealed:
            reveal=QPushButton("显示释义"); reveal.setToolTip("快捷键：空格或 Enter"); reveal.setStyleSheet(button_style("todo",True)); reveal.clicked.connect(self.reveal_vocab_word); actions.addWidget(reveal)
        else:
    ...
    def todo_panel(self):
        ...
        if self.show_todo_review:self.todo_archive(layout,completed_rows,today)
        self.box.addWidget(panel)
    def todo_row(self,todo,kind,position,count,today):
        ...
        edit=QPushButton("编辑"); edit.clicked.connect(lambda _,t=todo:self.edit_todo(t)); line.addWidget(edit); remove=QPushButton("×"); remove.setFixedSize(27,27); remove.setToolTip("从本机彻底删除，不进入回顾"); remove.setStyleSheet("QPushButton{padding:0;border:0;background:transparent;color:#9aa1ad;font-size:17px} QPushButton:hover{background:#fee2e2;color:#dc2626}"); remove.clicked.connect(lambda _,t=todo:self.delete_todo(t)); line.addWidget(remove); return card
    def todo_archive(self,layout,completed_rows,today):
        months={}
        for todo in completed_rows:
            day=todo.get("done_date") or (todo.get("done_at") or "")[:10] or "日期未知"; months.setdefault(day[:7] if day!="日期未知" else day,{}).setdefault(day,[]).append(todo)
        for month,days in months.items():
            month_key="month:"+month; count=sum(len(rows) for rows in days.values()); month_text=(month.replace("-","年")+"月" if month!="日期未知" else month); expanded=month_key in self.todo_archive_expanded; button=QPushButton(("▾  " if expanded else "▸  ")+f"{month_text}    {count} 项"); button.setStyleSheet("QPushButton{text-align:left;background:#fffaf2;color:#765733;border:1px solid #eddfc8;border-radius:8px;font-weight:700}"); button.clicked.connect(lambda _,key=month_key:self.toggle_archive_group(key)); layout.addWidget(button)
            if not expanded:continue
            for day,rows in days.items():
                day_key="day:"+day; day_open=day_key in self.todo_archive_expanded; day_label=(day[5:].replace("-","月")+"日" if day!="日期未知" else day); child=QPushButton(("▾  " if day_open else "▸  ")+f"{day_label}    {len(rows)} 项"); child.setStyleSheet("QPushButton{text-align:left;margin-left:18px;background:white;color:#806b52;border:1px solid #eee4d5;border-radius:7px;font-size:10px}"); child.clicked.connect(lambda _,key=day_key:self.toggle_archive_group(key)); layout.addWidget(child)
                if day_open:
                    for todo in rows:card=self.todo_row(todo,"done",0,len(rows),today); card.setContentsMargins(18,0,0,0); layout.addWidget(card)
    def add_todo(self):
    ...
    def reorder_todo_drag(self,todo_id,target):
        rows=sorted((t for t in self.todos if not t.get("done") and not t.get("active") and t.get("bucket","today")=="today"),key=lambda t:t.get("order",0)); source=next((i for i,t in enumerate(rows) if t.get("id")==todo_id),None)
        if source is None:return
        todo=rows.pop(source)
    ...
    def toggle_archive_group(self,key):
        if key in self.todo_archive_expanded:self.todo_archive_expanded.remove(key)
        else:self.todo_archive_expanded.add(key)
        self.refresh(scan_windows=False)
