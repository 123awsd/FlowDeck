...
“单词闪卡”复用用户自有项目 `123awsd/win-floating-vocab` 的三套词库和 32 张猫猫 PNG，并按当前 Qt 界面重新实现。默认先隐藏释义，要求主动回想后再翻面；选择“忘了 / 模糊 / 记住了”会安排不同的下次复习时间，忘记的词还会在当前学习轮次稍后再次出现。支持自然英语朗读（断网时使用本机语音）、收藏、顺序/随机、键盘操作和导入 UTF-8 TXT 词库。内置词库进入 Git，个人词库与复习记录只保存在 `data/`。
