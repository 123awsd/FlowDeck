...
项目卡片会以一行摘要显示当前对话的估算上下文占用、缓存命中、本轮输入/输出和运行时间，不在默认页面暴露对话名称；点击“详情”可查看累计、输入、缓存、新增输入、输出、推理 Token、缓存复用率以及同项目的其他对话。统计只读自本机 Codex 会话，文件尾读取有 4 MB 上限，不会复制或上传对话内容。
...
网络详情支持手动执行三类操作：“网络诊断”检查 Codex、GitHub、Hugging Face 可达性并读取 Mihomo 活动代理链；“当前线路测速”测量约 3 MB 下载、128 KB 上传、TTFB、稳定性和失败率；“推荐节点”最多测试 4 个存活候选节点，分别给出适合 Codex、GitHub 和 Hugging Face 的低延迟节点。支持 TCP 和 Unix Socket 控制接口，不会自动切换节点；测速与推荐只在内存中缓存 10 分钟，重启后清空。
