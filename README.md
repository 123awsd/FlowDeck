...
系统级 `Alt+Q` 和 `Alt+E` 分别对当前选中文字执行 DeepSeek 翻译和概念解释，结果会以紧凑聊天气泡显示在悬浮球旁。翻译保留 Piper 英文自动朗读，并支持重播和复制；选区只通过当前用户可访问的本机 Socket 瞬时传递，不写入本地历史。主程序未运行时自动回退到原有独立弹窗。

界面采用“轻卡通工作台”视觉系统：暖白表面配合任务蓝、待办杏、学习紫和系统绿四个低饱和分区，顶部与悬浮球使用本地猫猫素材，导航使用本地 SVG 图标。公共视觉 Token 位于 `src/codex_control_tower/theme.py`，完整规范见 [`docs/VISUAL_SYSTEM.md`](docs/VISUAL_SYSTEM.md)。视觉层不改变既有功能和本地数据逻辑。

## 工作与灵感管理

“我的待办”使用“正在做 + 今日 + 稍后”的轻量结构。今日事项通过卡片右侧手柄直接拖动排序，稍后用于收纳还不需要安排的长期事项；每项都可选截止日期，到期时自动进入今日。多个事项可同时设为“正在做”并统一置顶。完成项按“月份 → 日期”折叠收纳在本地回顾中；手动删除则会彻底从本地移除。
...
项目卡片会用清晰的浅蓝指标带显示当前对话的估算上下文占用、缓存命中、完整一轮输入/输出和运行时间，不在默认页面暴露对话名称；点击“详情”可查看累计、输入、缓存、新增输入、输出、推理 Token、缓存复用率以及同项目的其他对话。完整一轮优先采用 Codex 的 `turn_token_usage`，不会把最后一次模型请求误当成整轮。统计只读自本机会话，文件尾读取有 4 MB 上限，不会复制或上传对话内容。

任务页底部会汇总本机所有 Plus 与 API 运行时自每天 06:00 以来的 Token 消耗，显示总量、输入、输出和缓存命中率。统计只读每次请求的增量记录，不创建额外日流水。
...
“系统监控”页面使用本机 Linux 的 procfs/sysfs，不依赖常驻后台服务，每 2 秒刷新 CPU、内存、Swap、磁盘、网络、高占用进程和 NVIDIA GPU/显存数据。网络摘要显示实时上传/下载、当前网卡和 Mihomo/Clash 状态；“详情”再显示节点、TUN、系统代理和终端代理状态。磁盘区域会枚举当前所有有效的物理、网络和可移动挂载点，并过滤 proc、tmpfs、Snap 镜像等系统伪文件系统、EFI/boot 启动分区，以及包含 Windows 系统目录的 NTFS 系统盘。页面会显示最近样本的迷你曲线，曲线只保存在内存中，不会不断写入硬盘。当驱动或传感器不可用时，对应项目显示“数据不可用”，不弹窗、不干扰训练任务。

网络详情提供“网络诊断”和“当前线路测速”，以及 `Codex 加速`、`代码下载加速`、`模型下载加速`三个按钮。加速按钮在对应 Mihomo 策略组中实测候选节点并直接切换到目标服务延迟最低的节点；当前 GitHub 与 Hugging Face 共用主策略组，所以二者以最后点击的按钮为准。支持 TCP 和 Unix Socket 控制接口；测速结果只在内存中缓存 10 分钟，重启后清空。
