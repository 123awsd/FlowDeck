...
- 待办允许同时激活多项，并使用“今日拖动排序 + 稍后收纳箱 + 可选截止日期”的轻量结构；到期项自动进入今日，不自动激活。完成记录按月份和日期两级折叠。
- 完成待办进入本地回顾；删除则必须从本地彻底移除，不保留回收记录。已创建待办可修改内容、今日/稍后、截止日期和绑定项目。
...
完整设计见 [视觉系统](VISUAL_SYSTEM.md)、[工作与灵感管理](WORK_MANAGEMENT.md)、[账号与 API 服务商切换](ACCOUNT_PROVIDERS.md)、[对话与网络可观测性](OBSERVABILITY.md)、[领域学习系统](LEARNING_SYSTEM.md) 和 [单词闪卡](VOCABULARY.md)。
...
- 2026-09-08：任务页底部增加每日 Token 汇总，以本地时间 06:00 为日切分点，只读聚合所有 Plus/API 会话的请求级增量，不写入额外历史数据。
- 2026-09-11：待办改为“今日 + 稍后 + 可选截止日期”，今日项可手动排序，完成项进入有界本地回顾；删除项直接从本地移除。
- 2026-09-11：今日项由上下按钮升级为拖动手柄排序；完成记录改为“月份 → 日期 → 任务”折叠层级，大量历史不再直接铺开。
- 2026-09-11：每日 Token 统计改为按 JSONL 文件偏移量增量读取；长会话追加时不得在 UI 刷新中重扫整个文件。
- 2026-09-11：VS Code/远程窗口的 5 秒周期扫描移出 UI 主线程；扫描期间界面保持可交互，完成后再合并最新状态。
- 2026-09-11：页签切换不再同步扫描 VS Code；任务页先渲染已缓存的窗口快照，再由后台扫描校正，避免从待办返回时阻塞 UI。
- 2026-09-11：今日待办拖动使用跟手快照、实时占位换序、其他卡片短让位动画、边缘自动滚动和松手吸附；仅在跨过任务中线时启动有限数量动画，临时元素在拖动后释放，松手只写盘一次。
