...
- 本地桥接需以 VSIX 正常安装并随仓库版本自动更新。Plus 账号之间由 Codex Switch 在目标窗口内切换；Plus 与 API 之间必须通过独立 VS Code 主进程重开目标项目，因为同一主进程不能可靠地为单窗口隔离 `CODEX_HOME`。
...
- 2026-09-07：已废弃“在现有 VS Code 主进程内修改单窗口环境/启动包装器”的方案，因为 Codex 扩展不会稳定继承该环境，会出现界面显示 API 而实际仍用订阅。现改为每个 API 服务商使用独立 VS Code 用户数据目录和独立主进程；已通过 `/proc` 确认 Codex app-server 子进程继承了目标 `CODEX_HOME`，桥接升级至 0.1.7。
