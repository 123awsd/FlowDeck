...
- API Key 通过 Codex 自带的 `login --with-api-key` 一次性写入各服务商的独立目录；重启 Control Tower、VS Code 或电脑后继续复用，无需重复输入。界面只有在认证模式为 `apikey` 且本地密钥存在时才把服务商标记为可用。
...
- 2026-09-07：将 Heju 与 FastLab 密钥持久化到各自独立的本地 `auth.json`，并把“API 已配置”从文件存在检查升级为 API Key 认证状态检查，避免 ChatGPT OAuth 文件被误判为可用 API。
